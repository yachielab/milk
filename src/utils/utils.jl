...
# function rejection_sampling(input_dict,threshold,metric)
#     distance_function = map_distance_function(metric)
#     representative_dict = Dict{String,Vector{Float32}}()
#     groups = Dict{String,Vector{String}}()
#     n_comparisons = 0
#     for (candidate_id,candidate_vec) in input_dict
#         closest_id = nothing
#         closest_dist = Inf
#         for (representative_id,representative_vec) in representative_dict
#             dist = distance_function(representative_vec,candidate_vec)
#             n_comparisons += 1
#             if (dist <= threshold) && (dist < closest_dist)
#                 closest_id = representative_id
#                 closest_dist = dist
...
function optimization_process(;ids,vecs,D,index_map,groups,optimization_set,cache_dict,threshold,distance_function)
    ...
    # if !isempty(unmapped_dict)
    #     @warn "\t$(length(unmapped_dict)) object(s) could not be mapped to optimized representatives."
    # end
...
function process(;input_path,label,cache_path,previous_groups_path,invariant_args)
    ...
    @info "\tConcatenation across partitioned results"
    # concat_files = concatenate_partitioned_results(label,partition_dir,invariant_args["output-dir"])
    representatives_path,groups_path = concatenate_partitioned_results(label,partition_dir,invariant_args["output-dir"])

    # @info "Merge concatenated results (if tractable)..."
    # merged_files = merge_partitioned_results(concat_files,label,cache_path,previous_groups_path,invariant_args)

    # representatives_path = concat_files[4]
    # groups_path = concat_files[5]
